...
Als Alternative haben wir einmal den Göttinger Kiessee und einmal in Northeim den Großen See vermessen, diese sind aber von Menschen geschaffene Seen.\\
...
\label{sec:abs}
%Motivation:
Wenn Reflexion und die Mitbewegung des Mediums (Wasser) vernachlässigt werden, dann sollten sich die Abhängigkeiten bei der Ausbreitung des Schalls vom Piezokristall, ähnlich verhalten wie die Gesetze einer punktförmigen Schallquelle.
...
Entscheidend ist hier nur die Formel für den Verstärkungsfaktor $v$ für den verwendeten nichtinvertierenden Verstärkers. 
Für diese gilt
\begin{align}
...
Mit der Formel \eqref{eq:verst} ergibt sich bei der in der Abbildung \ref{fig:PT1000Schaltung} gezeigten Beschaltung ein Verstärkungsfaktor von 11.
...
Da in den See nicht viel Licht eintritt, wird die sich ergebende Spannung noch über einen nicht-invertierenden Verstärker nach der Gleichung \ref{eq:verst} mit einem Faktor 11 verstärkt.
...
Um die LEDs von dem Umgebungslicht zu unterscheiden werden diese gepulst, sie werden in einem bestimmten Zyklen ein und aus geschaltet.
Die Frequenz, mit der die LEDs nacheinander ein und aus geschaltet werden, die sogenannte Umschaltfrequenz, ist von 0\si{\hertz} bis 20\si{\hertz} durchschaltbar.
...
Hierzu werden 20 Schwingungen auf den Senderkristall gegeben.\\
