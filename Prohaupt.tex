...
Wichtig ist nun, dass bei flüssigem Wasser der Rotationsfreiheitsgrad ausscheidet, da die Moleküle durch die O-H Brücken zu stark gebunden sind.
...
Infrarotstrahlung regt nun gerade eine Kombination von zwei Vibrationszuständen an. Im allgemeinen Fall liegen aber insbesondere im sichtbaren Spektrum Energiebänder vor. Weil diese Zustände aber nun beliebig kompliziert werden können, muss man auf makroskopischer Ebene spektroskopische Versuche durchführen, um die Absorbtionspektren zu untersuchen.

In \ref{fig:abs} ist hierzu der Absorptionskoeffizient von reinem Wasser in einem Wellenlängenbereich von 380 bis 700 nm dargestellt. Dies entspricht gerade dem
sichtbaren Bereich.
Und noch einmal im Infraroten Bereich....

\begin{figure}[h]
	\centering
	\includegraphics[height=10cm]{abswater/abs}
	\caption{'Absorptionsspektrum (380-700nm) von reinem
		  Wasser. (1997). http://omlc.org/spectra/water/abs/}
	\label{fig:abs}
\end{figure}
...
Dies geschieht mithilfe eines nichinvertierenden Verstärkers. %ref aufquelle
...
Hierzu wird ein GPS von %Name der Firma und Produktbezeichnung hinzufügen
...
Zur Messung eines Sees ist es notwendig, zunächst den Bereich, den man vermessen möchte sinnvoll einzuschränken und sich eine Rasterung zu überlegen.
...
Die eigentliche Messung besteht darin, dass der Ruderer an einer Stelle anhält und gegebenenfalls den Messstab für das Geodimeter hochhält.
...
Als Vergleich sind die Daten, die wir mit einer anderen App aus dem App-Store\registerd aufgenommen haben in Abb. \ref{fig:GPSGoe} aufgetragen.
...
\begin{figure}[h]
\centering
\input{Messung\ Kiessee/karteGoe}
...
\begin{figure}[h]
\centering
\input{Messung\ Kiessee/GPSvsGeo}
