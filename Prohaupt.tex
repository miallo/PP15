...
\usepackage{graphicx}
\graphicspath{{GoeKiessee/}{Northeim/}{Theorie/}{Schall-Abstand/}}
...
\begin{figure}[h]
	...
	\caption{https://upload.wikimedia.org/wikipedia/commons/thumb/f/ff/Noninverting_Amplifier.svg/2000px-Noninverting_Amplifier.svg.png}
	\label{fig:nicht}
\end{figure}
...
Die Referenzspannung wird bei jedem Schritt halbiert oder um die Hälfte erhöht jeh nach dem, ob die zu messende Spannung kleiner oder größer ist.
...
Um die Absorption in verschiedenen Spektren zu Messen hat die Messsonde eine Blaue und eine Infrarote LED %ref auf die Datenblätter
mit mittleren Wellenlängen von $\lambda_\text{Blau}=\si{\nano\meter}$ und $\lambda_\text{IR}=\si{\nano\meter}$. %ref auf Datenblatt
Die LEDs werden ebenfalls von der board-internen Spannungsquelle versorgt und wie in der Abbildung \ref{fig:LEDSchaltung} zu sehen mit entsprechenden Vorwiderständen beschaltet.\\
...
Bei der Messbrücke handelt es sich um  eine Weathstonesche Brücke, die bereits vom Werk aus geeicht ist.
...
Dies ist über das Einstellungsmenü bei einmaligem Empfangen der Daten möglich.
...
Desweiteren erhitzen sich die Bautele bei diesen Spannungen schnell und es müssen Kühlkörper oder eine aktive Kühlung verwendet werden.\\
...
Außerdem behindert am Northeimer See eine Insel ein Großteil des Blickfelds.
...
Dies machte ein sonnst notwendiges Sortieren der Datein nach Ort und Zeit unnötig.
...
Dies  entspricht in etwa der Frequenz, mit der wir die LEDs geschaltet haben (keine LED, blaue LED, keine LED, IR-LED).
...
Glücklicherweise sind die Daten so verteilt, dass nur etwa 50\% Fehlerhaft sind und diese nur gefilter werden mussten.
...
Desweiteren ware die beide LEDs zur Messung der Absorbtion in verschiedenen Spektren nicht nah genug an der Photodiode, um eine signifikate Änderung an den Werten zu führen, die nicht im Rauschen untergehen.
...
Laut Datenblatt liegt der unterschied in der Intensität zwischen den beiden Wellenlängen bei 40\%, was sich auch in der Auswertung bemerkbar machte. %refferez auf Datenblatt
