...
class Game(object):
    ...
    def __init__(self, screen):
        ...
        self.background_getter = ResourceCapsule(self.resource_manager, 'image_background-hd.png')
        ...
        self.score_label_surface_getter = ResourceCapsule(self.resource_manager, "scorefont_Score")
        self.combo = 0
        self.combo_hits = 0
        self.combo_surface_getter = ResourceCapsule(self.resource_manager, "scorefont_"+str(self.combo))
        self.combo_hits_surface_getter = ResourceCapsule(self.resource_manager, "scorefont_("+str(self.combo_hits)+" hits)")
    ...
    def loading_init(self):
        ...
        self.background_getter().blit(self.score_label_surface_getter(), (29, 63))
        self.background_getter().blit(self.score_combo_plus_sign_surface_getter(), (47, 109))
    ...
    def draw(self, update=True):
        ...
        if self.game_over:
            self.screen_getter().blit(self.lives_surface_getter(), (16, 10))
        else:
            self.screen_getter().blit(self.lives_surface_getter(), (66, 10))
        self.screen_getter().blit(self.score_surface_getter(), (58 - self.score_surface_getter().get_width() // 2, 92))
        self.screen_getter().blit(self.combo_surface_getter(), (58 - self.combo_surface_getter().get_width() // 2, 153))
        self.screen_getter().blit(self.combo_hits_surface_getter(), (58 - self.combo_hits_surface_getter().get_width() // 2, 182))
    ...
    def dropped_balls(self):
        ...
        if self.lives > 0:
            self.add_ball()
            self.text_messages.append(widgets.TextMessage(self.screen_getter(), "Press SPACE to begin", Vector(self.screen_getter().get_width() / 2, self.screen_getter().get_height() / 2), duration=1800, size=32, initialdelay=800))
        else:
            self.lost()
        self.lives_surface_getter = ResourceCapsule(self.resource_manager, "livesfont_" + str(self.lives))
    ...
    def lost(self):
        ...
        self.lives = "DEAD"
        if android:
            self.text_messages.append(widgets.TextMessage(self.screen_getter(), "You died. Play again? Y/n", Vector(self.screen_getter().get_width() / 2, self.screen_getter().get_height() / 2 - 120), duration=9999999, size=32, initialdelay=9999990))
            android.show_keyboard()
        else:
            self.text_messages.append(widgets.TextMessage(self.screen_getter(), "You died. Play again? Y/n", Vector(self.screen_getter().get_width() / 2, self.screen_getter().get_height() / 2), duration=9999999, size=32, initialdelay=9999990))
...
class Menu(object):
    ...
    def main(self):
        ...
        if saveFileNotCorrupt():
            offset = 75
            newTitle = "New (save will be lost)"
